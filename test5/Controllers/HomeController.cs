...
namespace test5.Controllers
{
    public class HomeController : Controller
    {
        ...
        public HomeController(IOptions<AppSetings> options)
        {
        ...
        public async Task<IActionResult> Index(
            [FromServices] InventoryContext dbContext,
            [FromServices] IMemoryCache cache)
        {
            var cacheKey = "products";
            List<Inventory> inventory;
            if (!cache.TryGetValue(cacheKey, out inventory))
            {
                inventory = await GetInventoryAsync(dbContext, 6);
        ...
        public Task<List<Inventory>> GetInventoryAsync(InventoryContext dbContext, int count)
        {
            return dbContext.Inventory
                .ToListAsync();
        }
    }
}
